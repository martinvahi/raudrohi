...
window.raudrohi.dbcomm.dbq1=function(){
	var self_public_=this;
	try{

		this.verify_bin_query_ht=function(ht_query_candidate){
			if(raudrohi_settings_debug_JavaScript===true){
				window.raudrohi.base.assert_isObject(ht_query_candidate,
					'ht_query_candidate',
		...
		this.create_empty_ht=function(){
			try{
				var ht_q=window.raudrohi.base.pool_of_hashtables.get_empty_hashtable();
				var ht_selector=window.raudrohi.base.pool_of_hashtables.get_empty_hashtable();
				...
				window.raudrohi.tmg('6ca9a330-0490-4ec7-b297-6150a0219bd7',err);
			} // catch
		} // create_empty_ht
		...
		this.get_query_params_signature=function(ht_q){
			try{
				...
				var signature=window.raudrohi.lang.hashtable_signature(ht_selector);
				signature+=':';
				...
				window.raudrohi.tmg('4c10c421-14de-40f8-9297-6150a0219bd7',err);
			} // catch
		} // serialize_query_ht
		...
		this.serialize_query_ht=function(ht_q){
			try{
				...
				var s_ht_selector=window.raudrohi.lang.ht2ProgFTE(
					ht_q.get('ht_selector'));
				ht_q.put('ht_selector',s_ht_selector);
				//				ht_q.put('signature',
				//					self_public_.get_query_params_signature(ht_q));
				var s_progfte=window.raudrohi.lang.ht2ProgFTE(ht_q);
				ht_q.put('ht_selector',ht_tmp);
				return s_progfte;
			} catch (err){
				window.raudrohi.tmg('26870919-f84e-4ea2-a497-6150a0219bd7',err);
			} // catch
		} // serialize_query_ht
		...
		this.destroy=function(ht_q){
			try{
				self_public_.verify_bin_query_ht(ht_q);
				window.raudrohi.base.pool_of_hashtables.return_used_hashtable(ht_q.get('ht_selector'))
				window.raudrohi.base.pool_of_hashtables.return_used_hashtable(ht_q);
			} catch (err){
				window.raudrohi.tmg('454b6520-f7de-4e4c-a197-6150a0219bd7',err);
			} // catch
		} // destroy

	} catch (err){
		window.raudrohi.tmg('cecae120-a16f-44ae-a397-6150a0219bd7',err);
	} // catch
} // window.raudrohi.dbcomm.dbq1
window.raudrohi.dbcomm.dbq1=new window.raudrohi.dbcomm.dbq1();

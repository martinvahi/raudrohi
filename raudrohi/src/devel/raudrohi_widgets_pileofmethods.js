...
//       a clear distinction.
//
raudrohi.widgets.g1.pileofmethods_t1 = function(instance_public,
	...
	try {
		...
		try {
			...
			prc_.startup_shutdown_handler_ = new raudrohi.lang.startup_shutdown_handler(instance_public_);
			...
			instance_public_.only_for_raudrohi_core_developers.ht_microsessions_ =raudrohi.base.pool_of_hashtables.get_empty_hashtable();
			instance_public_.only_for_raudrohi_core_developers.ht_microsessions_cnt_ = 1;
			...
			instance_public_.only_for_raudrohi_core_developers.ht_microsessions_queue_ = new raudrohi.lang.htQueue();
			...
			instance_public_.state_switcher_ = new raudrohi.base.state_switcher_t1();
			instance_public_.change_state_2 = instance_public_.state_switcher_.change_state_2;
			...
			prc_.set_config_hook = function(config_field_name, config_field_value) {};
		...
		prc_.critical_section_rw=ob_critical_section_impl_.critical_section_rw;
		prc_.critical_section_r=ob_critical_section_impl_.critical_section_r;

		instance_public_.only_for_raudrohi_core_developers.content_from_GUI_2_vars_recursive = function(){
			try {
				...
				for (i = 0; i < len; i++) {
					...
					try {
						...
						raudrohi.tmg('a5c8f9ca-586d-4a1f-860f-13202011abd7', err);
					} // catch
				} // for
			}
			catch (err){
		...
		prc_.assert_stateExists = function(state_name) {
			try{
				...
				if (instance_public_.state_switcher_.state_exists(state_name) !== true) {
		...
		prc_.customizable.optional.hide_suffixhook= function(true_if_update_DOM) {}
		...
		instance_public_.hide = function(true_if_update_DOM) {
			try {
				...
				raudrohi.widgets.g1.sys.hide_or_unhide_t2(prc_.ar_widgets_,
					false, true_if_update_DOM, prc_.ht_widgets_conserved_states_);
		...
		instance_public_.unhide = function() {
			try {
				...
				if ((prc_.startup_shutdown_handler_.machine_is_running()) === false) {
					return;
				} // if
				...
				if (prc_.self_is_hidden_ === false) {
					...
					var pfawl = prc_.ar_widgets_.length; //A FireFox 3.0.x bug workaround.
					if (0 < pfawl) {
						var true_if_unhide = false;
						// The "hiding" and "unhiding" operations have
						// to be interlaced. Hence the hiding here.
						try {
							raudrohi.widgets.g1.sys.hide_or_unhide_t2(prc_.ar_widgets_,
								true_if_unhide, false,
		...
		instance_public_.get_widget_state = function() {
			try {
				...
				widget_state.is_in_state_startup = prc_.startup_shutdown_handler_.machine_is_running();
				widget_state.is_in_state_hidden = prc_.self_is_hidden_;
				widget_state.is_in_state_readonly = prc_.is_readonly_;
				return widget_state;
			} catch(err) {
		...
		prc_.customizable.optional.from_readonly_to_editable_prefixhook= function() {}
		prc_.customizable.optional.set_readonly_suffixhook= function(true_if_readonly) {}

		instance_public_.set_readonly = function(true_if_readonly) {
			try {
				// TODO: the set_readonly should be with
				// registration just like the hiding/unhiding is.
				// The element state bitfield has already been updated.
				if (prc_.self_is_graphical_widget_ !== true) {
					// The readonly state is a graphical phenomena.
					return;
				} // if
				if (prc_.is_readonly_ === true_if_readonly) {
					return;
				} // if
				if (prc_.startup_shutdown_handler_.machine_is_running()===true) {
				...
				prc_.is_readonly_ = true_if_readonly;
				...
				for (i = 0; i < len; i++) {
					elem = prc_.ar_widgets_[i];
					try {
						elem.set_readonly(true_if_readonly);
					} catch(err) {
						raudrohi.tmg('5ab3a87a-d756-4964-a5ee-13202011abd7', err);
					} // catch
				} // for
				prc_.customizable.optional.set_readonly_suffixhook(true_if_readonly);
			} catch(err) {
		...
		instance_public_.set_keylistener = function(key_number_as_string,
			...
			try {
				...
				if (!prc_.keylisteners_unit_inited_) {
					prc_.keylisteners_unit_ = new raudrohi.widgets.g1.sys.keylisteners_unit(instance_public_);
		...
		instance_public_.only_for_raudrohi_core_developers.widget_startup_sequence = function(){
			try {
				if (prc_.startup_shutdown_handler_.hook_startup_started()===true) {
					return;
				} // if
		...
		// is meant only for the raudrohi.lang.startup_shutdown_handler
		instance_public_.only_for_raudrohi_core_developers.widget_shutdown_sequence = function(){
			try {
				if (prc_.startup_shutdown_handler_.hook_shutdown_started()===true) {
					return;
				} // if
				instance_public_.state_switcher_.change_state_2('zero');
				raudrohi.widgets.g1.sys.change_state_2(prc_.ar_widgets_, 'zero');
		...
		prc_.register_subwidget = function(a_widget,
			...
			try {
				...
				switch (default_visibility_after_the_parent_widget_has_been_unhidden) {
					case 'hidden':
		...
		prc_.mark_elemwidget_to_be_hidden = function(a_widget, true_if_update_DOM) {
			try {
				...
				if (raudrohi_settings_debug_JavaScript === true) {
					...
					if (prc_.ht_widgets_conserved_states_.containsKey(phn)===false) {
		...
		function init_ht_widgets_conserved_states_if_necceccary() {
			try {
				...
				if (prc_.ht_widgets_conserved_states_default_.size() != prc_.ht_widgets_conserved_states_.size()) {
					raudrohi.widgets.g1.sys.copy_ht_of_bitfields(
						prc_.ht_widgets_conserved_states_,
						prc_.ht_widgets_conserved_states_default_);
				} // if
		...
		instance_public_.only_for_raudrohi_core_developers.microsession_set_id=function(ht_wrapper){
			try{
				...
				instance_public_.phone.get_phone_number()+'_'+raudrohi.base.generate_id();
				...
				instance_public_.only_for_raudrohi_core_developers.ht_microsessions_.put(microsession_name, cnt1);
				...
				var x_threshold=100;
				if (x_threshold<instance_public_.only_for_raudrohi_core_developers.ht_microsessions_queue_.length()) {
					...
					var ms_name;
					for(i=0;i<n;i++){
						ms_name = instance_public_.only_for_raudrohi_core_developers.ht_microsessions_queue_.pop();
			...
			try {
				if (raudrohi_settings_debug_JavaScript === true) {
					...
					raudrohi.base.assert_isString(destination_command,
						'destination_command', '2c996d1a-f4ae-4a43-838e-13202011abd7');
				} // if
		...
		prc_.threadjump_send = function(s_destination_phonenumber,
			...
			try {
				if (raudrohi_settings_debug_JavaScript === true) {
					...
					raudrohi.base.assert_isString(s_return_command,
						's_return_command', '977468f2-f0c4-43f2-837e-13202011abd7');
				} // if
				var ht_wrapper = instance_public_.only_for_raudrohi_core_developers.microsession_assemble_data_wrapper_hashtable(
		...
		prc_.threadjump_send_reply = function(
			...
			try {
				...
				ht_wrapper.put('return_command', 'threadjumper_ricochet_detector');
				var return_phonenumber_orig = ht_wrapper.get('return_phonenumber');
				ht_wrapper.put('destination_phonenumber', return_phonenumber_orig);
				ht_wrapper.put('return_phonenumber',
					instance_public_.phone.get_phone_number());
		...
		prc_.threadjump_redirect_outwards = function(s_destination_phonenumber,
			...
			try {
				if (raudrohi_settings_debug_JavaScript === true) {
					...
					raudrohi.base.assert_isString(s_return_command,
						's_return_command','d6cad926-1c3e-4c20-b26e-13202011abd7');
				} // if
				...
				ht_wrapper.put('return_command_redirect_'+phn, return_command_old);
				...
				ht_wrapper.put('destination_phonenumber', s_destination_phonenumber);
		...
		prc_.threadjump_redirect_inwards = function(ht_wrapper) {
			try {
				...
				ht_wrapper.put('destination_phonenumber', return_phonenumber_old);
				ht_wrapper.put('return_command', 'threadjumper_ricochet_detector');
				ht_wrapper.put('return_phonenumber', phn);
		...
		instance_public_.only_for_raudrohi_core_developers.microsession_receive_bin_t1 = function(ht_wrapper) {
			try {
				...
				if (instance_public_.only_for_raudrohi_core_developers.ht_microsessions_.containsKey(microsession_name)) {
					if (parseInt(microsession_id, 10) != instance_public_.only_for_raudrohi_core_developers.ht_microsessions_.get(microsession_name)) {
						return a_pair;
					} // if
				} // if
				instance_public_.only_for_raudrohi_core_developers.ht_microsessions_cnt_++;
		...
		prc_.microsession_dismiss_all = function() {
			try {
				...
				for (var i = 0; i < len; i++) {
					...
					microsession_cnt = instance_public_.only_for_raudrohi_core_developers.ht_microsessions_.get(
						microsession_name);
					// The +1 is as if we had already received the first package.
					cnt_plus = microsession_cnt + 1;
					instance_public_.only_for_raudrohi_core_developers.ht_microsessions_.put(microsession_name, cnt_plus);
				} // for
				prc_.microsession_dismiss_all_in_progress_ = false;
			} catch(err) {
		...
		this.standard_phonecall_received_t1 = function(a_pair, a_phonecall_instance) {
			...
			try {
				...
				switch (brancher) {
					...
					case 'startup_with_indication_request':
						...
						prc_.startup_shutdown_handler_.startbutton_pushed_with_indication_request(ht.get('destination_phone_number'), ht.get('data_for_the_destination'), ht.get('binary_data_for_the_destination'));
						break;
					case 'shutdown_with_indication_request':
						ht = a_phonecall_instance.data2;
						prc_.startup_shutdown_handler_.stopbutton_pushed_with_indication_request(ht.get('destination_phone_number'), ht.get('data_for_the_destination'), ht.get('binary_data_for_the_destination'));
						break;
					case 'startup':
						prc_.startup_shutdown_handler_.startbutton_pushed();
						break;
		...
		this.receive_phonecall_t1 = function(a_phonecall_instance) {
			try {
				...
				if (pileofmethods_t1_.standard_phonecall_received_t1(a_pair, a_phonecall_instance)) {
					return;
				} // if
		...
		instance_public_.phone.receive_phonecall=pileofmethods_t1_.receive_phonecall_t1;

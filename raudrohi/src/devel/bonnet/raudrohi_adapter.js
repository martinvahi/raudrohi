...
// window.raudrohi.initiate_adapter.
//
// Dependencies:
...
if (window.raudrohi.globals.raudrohi_adapter_exists !== true) {
    window.raudrohi.adapter = {};
    window.raudrohi.globals.raudrohi_adapter_exists = true;
} // if
if (window.raudrohi.globals.raudrohi_adapter_ports_exists !== true) {
    window.raudrohi.adapter.ports = {};
    window.raudrohi.globals.raudrohi_adapter_ports_exists = true;
} // if

window.raudrohi.internal_constructor = function () {
    throw 'It\'s assumed that the function named ' +
          'window.raudrohi.internal_constructor is overriden. ' +
          'GUID=="11695cd9-d655-486c-b4c4-93c210505dd7"';
} // window.raudrohi.internal_constructor

window.raudrohi.application_main_function = function () {
    throw 'It\'s assumed that the function named ' +
          'window.raudrohi.application_main_function is overriden. ' +
          'GUID=="fe8cdf13-5317-41bc-a5c4-93c210505dd7" ';
} // window.raudrohi.application_main_function

window.raudrohi.adapter.vars = {};

window.raudrohi.adapter.vars.func_if_lib_uninited = function (x) {
    throw 'In order to use the raudrohi ' +
          'JavaScript library, one has to initialize it by calling ' +
          'window.raudrohi.initiate_adapter. ';
} // window.raudrohi.adapter.vars.func_if_lib_uninited
...
window.raudrohi.adapter.log = function (x) {/*its a temporary stub*/
};

// The following 2 methods are subject to overriding. They're functions
// here only to demonstrate their interface.
window.raudrohi.adapter.server_response_eventhandler = function (response_as_a_text) {
    window.raudrohi.adapter.vars.func_if_lib_uninited('')
} // window.raudrohi.adapter.server_response_eventhandler
window.raudrohi.adapter.send2server = function (url_string, data_as_text) {
    window.raudrohi.adapter.vars.func_if_lib_uninited('')
} // window.raudrohi.adapter.send2server


// the innerHTML parameter exists only, because
// the Internet Explorer 8 (as of 11.2009) has bugs.
window.raudrohi.adapter.set_innerHTML = function (html_id, a_string) {
    window.raudrohi.adapter.vars.func_if_lib_uninited('')
} //window.raudrohi.adapter.set_innerHTML
...
window.raudrohi.tmg = function (GUID, err) {
    ...
    var msg = 'GUID=="' + GUID + "\"\n" + err;
    if (window.raudrohi.settings.runtime.write_exception_stack_2_log) {
...
window.raudrohi.adapter.YUI_stoppropagation_preventdefault =
window.raudrohi.adapter.vars.func_if_lib_uninited;

window.raudrohi.adapter.ports.selected_port_name = '';

//------------------------------------------------------------------------
window.raudrohi.adapter.vars.verify_event_name = function (event_name) {
    try {
        switch (event_name) {
            case 'focus':
                break;
            ...
            default:
                if (raudrohi_settings_debug_JavaScript === true) {
                    window.raudrohi.tmg('GUID=="1f5be820-20e8-4067-a3c4-93c210505dd7"',
...
} // window.raudrohi.adapter.vars.verify_event_name

//------------------------------------------------------------------------
window.raudrohi.adapter.vars.do_something_if_the_connection_to_the_server_failed =
function () {
    if (raudrohi_settings_debug_JavaScript === true) {
        window.raudrohi.tmg('fd893135-0aa3-43e0-93c4-93c210505dd7',
...
//------------------------------------------------------------------------
window.raudrohi.adapter.ports.yui_3_0 = function () {
    ...
    window.raudrohi.adapter.vars.YUI_3_0 = {};

    window.raudrohi.adapter.log = function (a_string) {
        if (raudrohi_settings_debug_JavaScript === true) {
            Y_raudrohi.log(a_string);
        } // if
    } // window.raudrohi.adapter.log

    window.raudrohi.adapter.trim = function (a_string) {
        try {
            return Y_raudrohi.Lang.trim(a_string);
        } catch (err) {
            window.raudrohi.tmg('2ddccaa1-e7e2-4510-95c4-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.trim

    window.raudrohi.adapter.YUI_preventdefault = function (e) {
        try {
            e.preventDefault();
        } catch (err) {
            window.raudrohi.tmg('961936f6-0928-43f5-b9c4-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.YUI_preventdefault

    window.raudrohi.adapter.YUI_stoppropagation = function (e) {
        try {
            e.stopPropagation();
        } catch (err) {
            window.raudrohi.tmg('aff2d145-4ba6-4d21-84c4-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.YUI_stoppropagation

    window.raudrohi.adapter.YUI_stoppropagation_preventdefault = function (e) {
        try {
            e.halt();
        } catch (err) {
            window.raudrohi.tmg('51c0580d-54d6-4943-b1c4-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.YUI_stoppropagation_preventdefault

    window.raudrohi.adapter.YUI_create_console = function () {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
    ...
    function (html_id, event_name, event_handler_func) {
        try {
            ...
            // the window.raudrohi.adapter.set_keylistener. The compatibility is
            // required in the window.raudrohi.widgets.g1.sys.keylisteners_unit.
            return key_listener_handle;
        } catch (err) {
            window.raudrohi.tmg('f9c22c5e-e20a-40a1-b4a4-93c210505dd7',
                err + ' html_id==' + html_id + '  event_name==' + event_name);
        } // catch
    } // window.raudrohi.adapter.addEventListner

    window.raudrohi.adapter.set_keylistener =
    function (html_id, key_number_as_string, event_handler_func) {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
                if (!raudrohi_adapter_isString(html_id)) {
                    window.raudrohi.tmg('29d851d1-ccea-4516-93a4-93c210505dd7',
                        '(!raudrohi_adapter_isString(html_id))==true')
                } // if
                if (!raudrohi_adapter_isString(key_number_as_string)) {
                    window.raudrohi.tmg('135e8c83-d464-4ed9-a7a4-93c210505dd7',
                        '(!raudrohi_adapter_isString(key_number_as_string))==true')
                } // if
                if (!raudrohi_adapter_isFunction(event_handler_func)) {
                    window.raudrohi.tmg('b8fe8932-3ff9-4a09-b3a4-93c210505dd7',
                        '(!raudrohi_adapter_isFunction(event_handler_func))==true');
                } // if
                ...
                if (!window.raudrohi.core.isWithinKeyeventKeyCodes(key_code_candidate)) {
                    window.raudrohi.tmg('a167b759-d3e8-444f-a1a4-93c210505dd7',
            ...
            window.raudrohi.tmg('863b8339-867c-45f3-a1a4-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.set_keylistener

    window.raudrohi.adapter.remove_keylistener = function (key_listener_handle) {
        try {
            key_listener_handle.detach();
        } catch (err) {
            window.raudrohi.tmg('4395f126-70e6-44a3-94a4-93c210505dd7', err);
        } // catch
    } // remove_keylistener

    window.raudrohi.adapter.JSON2ob = function (a_json_string) {
        try {
            var an_object;
            ...
            try {
                ...
                window.raudrohi.tmg("Invalid JSON string. " + e);
            }
            //});
            return an_object;
        } catch (err) {
            window.raudrohi.tmg('bd03611f-5fc8-425c-83a4-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.JSON2ob

    window.raudrohi.adapter.ob2JSON = function (an_object) {
        try {
            var s_json = '';
            ...
            window.raudrohi.tmg('ac3e7d35-80e7-43c1-a1a4-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.JSON2ob
    ...
    window.raudrohi.adapter.send2server = function (url_string, data_as_text) {
        try {
                ...
                try {
                    ...
                    if (data != null) {// IE problems
                        if (raudrohi_adapter_isString(data)) {
                            if (data != '') {
                                window.raudrohi.adapter.server_response_eventhandler(data);
                            } // if
                        } // if
                    } // if
                    //  var args = args[1]; // 'ipsum'.
                } catch (err) {
                    window.raudrohi.tmg('0805df11-3934-4d29-8394-93c210505dd7', err);
                } // catch
            } // request_complete
            ...
            var cfg = {
                method: 'POST',
                //data: 'debug|||/dev/null|||',
                timeout: window.raudrohi.settings.ajax_request_timeout * 1000,
                on: {
    ...
    window.raudrohi.adapter.getAttribute = function (html_id, attribute_name) {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
                if (!raudrohi_adapter_isString(html_id)) {
                    window.raudrohi.tmg('f9c2fa2d-af32-4781-9594-93c210505dd7',
                        '(!raudrohi_adapter_isString(html_id))==true')
                } // if
                if (!raudrohi_adapter_isString(attribute_name)) {
                    window.raudrohi.tmg('e0939914-1436-402c-a194-93c210505dd7',
                        '(!raudrohi_adapter_isString(attribute_name))==true')
                } // if
            } // if
            var node = Y_raudrohi.Node.get('#' + html_id);
            if (node === null) {
                window.raudrohi.tmg('75641d49-0544-4b18-a284-93c210505dd7',
                    ' node===null ');
            } // if
            var attr_value = node.get(attribute_name);
            return attr_value;
        } catch (err) {
            window.raudrohi.tmg('1b8cb863-044a-4e4d-9634-93c210505dd7', err +
                                                                 '  html_id==' +
                                                                 html_id +
                                                                 ' attribute_name==' +
                                                                 attribute_name);
        } // catch
    } // window.raudrohi.adapter.getAttribute

    window.raudrohi.adapter.remove_HTML_attribute =
    function (html_id, attribute_name) {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
                if (!raudrohi_adapter_isString(html_id)) {
                    window.raudrohi.tmg('6ed9d9b4-e72f-497e-9134-93c210505dd7',
                        '(!raudrohi_adapter_isString(html_id))==true')
                } // if
                if (!raudrohi_adapter_isString(attribute_name)) {
                    window.raudrohi.tmg('5ac26be2-eb73-4bee-9f34-93c210505dd7',
                        '(!raudrohi_adapter_isString(attribute_name))==true')
                } // if
            } // if
            var node = Y_raudrohi.Node.get('#' + html_id);
            if (node === null) {
                window.raudrohi.tmg('1b214c33-1cf8-4760-9224-93c210505dd7',
                    ' node===null ');
            } // if
            node.removeAttribute(attribute_name);
        } catch (err) {
            window.raudrohi.tmg('d6136558-0a48-4b41-b324-93c210505dd7', err +
                                                                 '  html_id==' +
                                                                 html_id +
                                                                 ' attribute_name==' +
                                                                 attribute_name);
    ...
    window.raudrohi.adapter.editStyle =
    function (html_id, style_param_name, style_param_value) {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
                if (!raudrohi_adapter_isString(html_id)) {
                    window.raudrohi.tmg('73ae0943-3ed9-4ff8-9224-93c210505dd7',
                        '(!raudrohi_adapter_isString(html_id))==true')
                } // if
                if (!raudrohi_adapter_isString(style_param_name)) {
                    window.raudrohi.tmg('a2110a35-367b-460c-8224-93c210505dd7',
                        '(!raudrohi_adapter_isString(style_param_name))==true')
                } // if
                if (!raudrohi_adapter_isString(style_param_value)) {
                    window.raudrohi.tmg('4bd680b1-d0b0-45e1-b124-93c210505dd7',
                        '(!raudrohi_adapter_isString(style_param_value))==true')
                } // if
            } // if
            var node = Y_raudrohi.Node.get('#' + html_id);
            if (node === null) {
                window.raudrohi.tmg('587a6034-ae54-4c02-8124-93c210505dd7',
                    ' node===null, html_id==' + html_id + "\n");
            } // if
            node.setStyle(style_param_name, style_param_value);
        } catch (err) {
            window.raudrohi.tmg('a36c260f-b2a0-4c05-b524-93c210505dd7', err +
                                                                 "\n html_id==" +
                                                                 html_id +
                                                                 "\n style_param_name==" +
                                                                 style_param_name +
    ...
    window.raudrohi.adapter.set_innerHTML = function (html_id, a_string) {
        try {
            var node = Y_raudrohi.Node.get('#' + html_id);
            if (node === null) {
                window.raudrohi.tmg('0ec81b12-4571-4ff4-9224-93c210505dd7',
                    ' node===null ');
            } // if
            node.set('innerHTML', a_string);
        } catch (err) {
            window.raudrohi.tmg('4ac5ef40-0a67-4c67-9414-93c210505dd7', err +
                                                                 '  html_id==' +
                                                                 html_id +
                                                                 ' a_string==' +
                                                                 a_string);
...
} // window.raudrohi.adapter.ports.yui_3_0

//------------------------------------------------------------------------

window.raudrohi.adapter.ports.yui_3_9_0 = function () {
    var self_public_ = this;
    this.get_name = function () {
        return 'YUI_3_9_0';
    } // get_name
    ...
    window.raudrohi.adapter.vars.YUI_3_9_0 = {};
    ...
    window.raudrohi.adapter.trim = function (a_string) {
        try {
            ...
            window.raudrohi.tmg('2d41fca4-057a-4cc5-8914-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.trim
    ...
    window.raudrohi.adapter.YUI_preventdefault = function (e) {
        try {
            ...
            window.raudrohi.tmg('20b0a651-7bf9-4d48-9314-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.YUI_preventdefault
    ...
    window.raudrohi.adapter.YUI_stoppropagation = function (e) {
        try {
            ...
            window.raudrohi.tmg('598ad635-a295-4ea9-8214-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.YUI_stoppropagation
    ...
    window.raudrohi.adapter.YUI_stoppropagation_preventdefault = function (e) {
        try {
            ...
            window.raudrohi.tmg('d2225755-caf5-43a5-8c14-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.YUI_stoppropagation_preventdefault
    ...
    window.raudrohi.adapter.YUI_create_console = function () {
        try {
            ...
            window.raudrohi.tmg('fafa4f48-fdd0-4108-b414-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.YUI_create_console
    ...
    function (html_id, event_name, event_handler_func) {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
                if (!raudrohi_adapter_isString(html_id)) {
                    window.raudrohi.tmg('b703066b-c61f-4c9a-b514-93c210505dd7',
                        '(!raudrohi_adapter_isString(html_id))==true')
                } // if
                if (!raudrohi_adapter_isString(event_name)) {
                    window.raudrohi.tmg('8e07f31d-7f9e-493a-b504-93c210505dd7',
                        '(!raudrohi_adapter_isString(event_name))==true')
                } // if
                if (!raudrohi_adapter_isFunction(event_handler_func)) {
                    window.raudrohi.tmg('3c5a2f2f-eb58-4591-8404-93c210505dd7',
                        '(!raudrohi_adapter_isFunction(event_handler_func))==true');
                } // if
                window.raudrohi.adapter.vars.verify_event_name(event_name);
            } // if
            ...
            window.raudrohi.tmg('07061723-9d66-4e8e-a204-93c210505dd7',
                err + ' html_id==' + html_id + '  event_name==' + event_name);
        } // catch
    } // window.raudrohi.adapter.addEventListner
    ...
    function (html_id, key_number_as_string, event_handler_func) {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
                if (!raudrohi_adapter_isString(html_id)) {
                    window.raudrohi.tmg('34015aed-d2e6-4723-9104-93c210505dd7',
                        '(!raudrohi_adapter_isString(html_id))==true')
                } // if
                if (!raudrohi_adapter_isString(key_number_as_string)) {
                    window.raudrohi.tmg('562ea14f-57d9-4534-8404-93c210505dd7',
                        '(!raudrohi_adapter_isString(key_number_as_string))==true')
                } // if
                if (!raudrohi_adapter_isFunction(event_handler_func)) {
                    window.raudrohi.tmg('321f9847-4952-4811-b504-93c210505dd7',
                        '(!raudrohi_adapter_isFunction(event_handler_func))==true');
                } // if
                // If the parseInt converts something like 'BumBastic' to
                // an int, then there will be trouble. It's not possible to
                ...
                var key_code_candidate = parseInt(key_number_as_string, 10);
                if (!window.raudrohi.core.isWithinKeyeventKeyCodes(key_code_candidate)) {
                    window.raudrohi.tmg('5f180351-0aa2-4a03-a3f3-93c210505dd7',
                        'key_number(==' + key_number_as_string +
                        ') does not represent a ' +
                        'JavaScript key event key code.');
                } // if
            ...
            window.raudrohi.tmg('657db51d-fc12-4880-94f3-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.set_keylistener
    ...
    window.raudrohi.adapter.remove_keylistener = function (key_listener_handle) {
        try {
            ...
            window.raudrohi.tmg('e9bcf51e-9ade-49f7-91f3-93c210505dd7', err);
        } // catch
    } // remove_keylistener
    ...
    window.raudrohi.adapter.JSON2ob = function (a_json_string) {
        try {
            ...
            window.raudrohi.tmg('2be3f983-81ca-4d7c-a6f3-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.JSON2ob
    ...
    window.raudrohi.adapter.ob2JSON = function (an_object) {
        try {
            ...
            window.raudrohi.tmg('39fb814d-4033-437f-83f3-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.JSON2ob
        ...
        try {
            ...
            function request_complete(id, o, args) {
                try {
                    ...
                    window.raudrohi.tmg('1e217366-3952-4283-91f3-93c210505dd7', err);
                } // catch
            } // request_complete
            ...
            window.raudrohi.tmg('4b863a2b-6492-4342-b4f3-93c210505dd7', err);
        } // catch
    } // window.raudrohi.adapter.send2server
    ...
    function (html_id, attribute_name, attribute_value) {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
                if (!raudrohi_adapter_isString(html_id)) {
                    window.raudrohi.tmg('3051db57-32dd-4475-92e3-93c210505dd7',
                        '(!raudrohi_adapter_isString(html_id))==true')
                } // if
                if (!raudrohi_adapter_isString(attribute_name)) {
                    window.raudrohi.tmg('71f53b43-9eac-47c2-8ce3-93c210505dd7',
                        '(!raudrohi_adapter_isString(attribute_name))==true')
                } // if
                if (!raudrohi_adapter_isString(attribute_value)) {
                    window.raudrohi.tmg('6b075645-a641-4af3-b4e3-93c210505dd7',
                        '(!raudrohi_adapter_isString(attribute_value))==true')
                } // if
            } // if
            var node = Y_raudrohi.Node.get('#' + html_id);
            if (node === null) {
                window.raudrohi.tmg('ad15043e-34cf-47fc-84e3-93c210505dd7',
                    ' node===null ');
            } // if
            node.set(attribute_name, attribute_value);
        } catch (err) {
            window.raudrohi.tmg('4697a86c-b81c-4850-a5e3-93c210505dd7', err +
                                                                 '  html_id==' +
                                                                 html_id +
                                                                 ' attribute_name==' +
                                                                 attribute_name +
    ...
    window.raudrohi.adapter.getAttribute = function (html_id, attribute_name) {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
                if (!raudrohi_adapter_isString(html_id)) {
                    window.raudrohi.tmg('2e0e2c14-d400-4511-b3e3-93c210505dd7',
                        '(!raudrohi_adapter_isString(html_id))==true')
                } // if
                if (!raudrohi_adapter_isString(attribute_name)) {
                    window.raudrohi.tmg('20098cb4-7f3a-4acb-afd3-93c210505dd7',
                        '(!raudrohi_adapter_isString(attribute_name))==true')
                } // if
            } // if
            var node = Y_raudrohi.Node.get('#' + html_id);
            if (node === null) {
                window.raudrohi.tmg('9290c7a1-ca41-4d87-92d3-93c210505dd7',
                    ' node===null ');
            } // if
            var attr_value = node.get(attribute_name);
            return attr_value;
        } catch (err) {
            window.raudrohi.tmg('75bf9f47-254c-42fc-83d3-93c210505dd7', err +
                                                                 '  html_id==' +
                                                                 html_id +
                                                                 ' attribute_name==' +
                                                                 attribute_name);
    ...
    function (html_id, attribute_name) {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
                if (!raudrohi_adapter_isString(html_id)) {
                    window.raudrohi.tmg('20756c61-2d21-4760-91d3-93c210505dd7',
                        '(!raudrohi_adapter_isString(html_id))==true')
                } // if
                if (!raudrohi_adapter_isString(attribute_name)) {
                    window.raudrohi.tmg('eddb5a2d-e373-4b42-a2d3-93c210505dd7',
                        '(!raudrohi_adapter_isString(attribute_name))==true')
                } // if
            } // if
            var node = Y_raudrohi.Node.one('#' + html_id);
            if (node === null) {
                window.raudrohi.tmg('225d9126-ada4-4d93-b4d3-93c210505dd7',
                    ' node===null ');
            } // if
            node.removeAttribute(attribute_name);
        } catch (err) {
            window.raudrohi.tmg('6339108f-2d53-4f6b-b5c3-93c210505dd7', err +
                                                                 '  html_id==' +
                                                                 html_id +
                                                                 ' attribute_name==' +
                                                                 attribute_name);
    ...
    function (html_id, style_param_name, style_param_value) {
        try {
            if (raudrohi_settings_debug_JavaScript === true) {
                if (!raudrohi_adapter_isString(html_id)) {
                    window.raudrohi.tmg('7a2a3646-30ed-434b-92c3-93c210505dd7',
                        '(!raudrohi_adapter_isString(html_id))==true')
                } // if
                if (!raudrohi_adapter_isString(style_param_name)) {
                    window.raudrohi.tmg('63b7773f-5727-4467-b5c3-93c210505dd7',
                        '(!raudrohi_adapter_isString(style_param_name))==true')
                } // if
                if (!raudrohi_adapter_isString(style_param_value)) {
                    window.raudrohi.tmg('4b819742-4fc0-4ae1-a4c3-93c210505dd7',
                        '(!raudrohi_adapter_isString(style_param_value))==true')
                } // if
            } // if
            var node = Y_raudrohi.Node.one('#' + html_id);
            if (node === null) {
                window.raudrohi.tmg('88261134-7021-48f2-91c3-93c210505dd7',
                    ' node===null, html_id==' + html_id + "\n");
            } // if
            node.setStyle(style_param_name, style_param_value);
        } catch (err) {
            window.raudrohi.tmg('b5224852-1019-4dd2-82c3-93c210505dd7', err +
                                                                 "\n html_id==" +
                                                                 html_id +
                                                                 "\n style_param_name==" +
                                                                 style_param_name +
    ...
    window.raudrohi.adapter.set_innerHTML = function (html_id, a_string) {
        try {
            var node = Y_raudrohi.Node.get('#' + html_id);
            if (node === null) {
                window.raudrohi.tmg('c0acbc52-be2b-4fb2-b3b3-93c210505dd7',
                    ' node===null ');
            } // if
            node.set('innerHTML', a_string);
        } catch (err) {
            window.raudrohi.tmg('5cb6c047-142f-48c1-95b3-93c210505dd7', err +
                                                                 '  html_id==' +
                                                                 html_id +
                                                                 ' a_string==' +
                                                                 a_string);
...
} // window.raudrohi.adapter.ports.yui_3_9_0
...
// kibuvits_writeln "port_name='"+RAUDROHI_PORT_NAME+"';"
...
// RENESSAATOR_BLOCK_END
                ...
                window.raudrohi.settings.debug_SERVERSIDE = window.raudrohi.core.str2bool(
                    elem.innerHTML);
                elem_id =
                ...
                raudrohi_settings_debug_JavaScript = window.raudrohi.core.str2bool(
                    elem.innerHTML);
                elem_id =
                ...
                if (elem !== null) {
                    ...
                    window.raudrohi.settings.ajax_request_timeout = seconds;
                } // if
            } // else
        } catch (err) {
            throw 'GUID=="85e32b2e-f35f-4ae4-91b4-93c210505dd7", ' + err;
        } // catch
        window.raudrohi.adapter.ports.selected_port_name = port_name;
        switch (port_name) {
            case 'YUI_3_9_0':
                window.raudrohi.adapter.ports.yui_3_9_0();
                break;

...
window.raudrohi.widgets.g1.multiindex_cache_t1=function(s_html_id,
	...
	try{
		...
		this.clear=function(){
			try{
				...
				window.raudrohi.tmg('3dbd3621-452c-48ac-8e2f-231350705dd7',err);
			} // catch
		} // clear
			...
			try{
				if(raudrohi_settings_debug_JavaScript===true){
					...
					if (ht_indices_.containsKey(s_index_name)===true){
						window.raudrohi.tmg('552563bd-4e07-483d-9a3f-231350705dd7',
							'Index with a name of "'+s_index_name+
							'" has already been declared.');
					} // if
				} // if
				var ht_new_index=window.raudrohi.base.pool_of_hashtables.get_empty_hashtable();
				ht_indices_.put(s_index_name,ht_new_index);
				var ht_new_index_complement_set=window.raudrohi.base.pool_of_hashtables.get_empty_hashtable();
				ht_indices_complement_sets_.put(s_index_name,
					ht_new_index_complement_set);
			} catch (err){
				window.raudrohi.tmg('34c59d35-17d2-4c1f-934f-231350705dd7',err);
			} // catch
		} // declare_index
		...
		this.insert_data_2_indices=function(ht_indices,x_data){
			try{
				window.raudrohi.tmg('5932d144-c791-461e-843f-231350705dd7',
					'This method is meant to be overridden, because the insertion '+
					'of data to the set of indices is application specific. ');
			} catch (err){
				window.raudrohi.tmg('93b72df0-a9c4-443d-a4af-231350705dd7',err);
			} // catch
		...
		this.convert_datasource_sent_datastring_to_cachable_form=function(s_data){
			try{
				...
				window.raudrohi.tmg('c30912d1-944a-40c8-ab5f-231350705dd7',err);
			} // catch
		...
		function send_response_to_query_initiator(ht_wrapper_with_query,x_data,b_data_found){
			try{
				...
				window.raudrohi.tmg('084edbae-4a30-41e7-8d3f-231350705dd7',err);
			} // catch
		...
		this.thrjr_.receive_from_datasource=function(ht_wrapper){
			try{
				...
				var b_data_found=window.raudrohi.core.str2bool(s_b_data_found);
				var x_data="";
				...
				window.raudrohi.tmg('3f7bc8e4-49f5-4d64-9d2f-231350705dd7',err);
			} // catch
			...
			try{
				...
				if(raudrohi_settings_debug_JavaScript===true){
					if (ht_indices_.containsKey(s_index_name)===false){
						window.raudrohi.tmg('51d53335-188e-4422-841e-231350705dd7',
							'Index with a name of "'+s_index_name+
				...
				var s_query_signature=window.raudrohi.lang.ht2ProgFTE(ht_query);
				var ar=null;
				...
				ar.push(ht_wrapper);
				var ht_data_2_datasource=window.raudrohi.base.pool_of_hashtables.get_empty_hashtable();
				...
				window.raudrohi.tmg('81743d1f-e5bf-408e-932e-231350705dd7',err);
			} // catch
		} // thrjr_.get

	} catch (err){
		window.raudrohi.tmg('2b976342-5376-4ccf-8b3e-231350705dd7',err);
	} // catch
} // window.raudrohi.widgets.g1.multiindex_cache_t1
